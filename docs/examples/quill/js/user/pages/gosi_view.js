/**
 * 공시사항 상세보기(gosi_view) 페이지 Initialization
 * src/main/resources/templates/user1/board/gosi_view.html용 JavaScript
 *
...
/**
 * 공시사항 상세보기 페이지 Alpine Data
 */
export function initGosiViewPage() {
    return {
        ...initAlpineBase(),
        ...
        /**
         * 첨부파일 로드
         * GET /api/files/post/gosi/{postId}
         */
        loadAttachedFiles() {
            ...
            fetch(`/api/files/post/gosi/${this.postId}`)
                .then(response => {
                    if (!response.ok) {
        ...
        /**
         * 이전/다음 글 정보 로드
         * GET /user1/api/board/gosi/{id}/navigation
         */
        loadNavigationPosts() {
            if (!this.postId) return;
            ...
            fetch(`/user1/api/board/gosi/${this.postId}/navigation`)
                .then(response => {
                    if (!response.ok) {
        ...
        previousPost() {
            if (this.previousPostId) {
                window.location.href = `/user1/board/gosi/${this.previousPostId}`;
            }
        },
        ...
        nextPost() {
            if (this.nextPostId) {
                window.location.href = `/user1/board/gosi/${this.nextPostId}`;
            }
        },
