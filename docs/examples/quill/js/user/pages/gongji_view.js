/**
 * 공지사항 상세보기(gongji_view) 페이지 Initialization
 * src/main/resources/templates/user1/board/gongji_view.html용 JavaScript
 *
...
/**
 * 공지사항 상세보기 페이지 Alpine Data
 */
export function initGongjiViewPage() {
    return {
        ...initAlpineBase(),
        ...
        /**
         * 첨부파일 로드
         * GET /api/files/post/gongji/{postId}
         */
        loadAttachedFiles() {
            ...
            fetch(`/api/files/post/gongji/${this.postId}`)
                .then(response => {
                    if (!response.ok) {
        ...
        /**
         * 이전/다음 글 정보 로드
         * GET /user1/api/board/gongji/{id}/navigation
         */
        loadNavigationPosts() {
            if (!this.postId) return;
            ...
            fetch(`/user1/api/board/gongji/${this.postId}/navigation`)
                .then(response => {
                    if (!response.ok) {
        ...
        previousPost() {
            if (this.previousPostId) {
                window.location.href = `/user1/board/gongji/${this.previousPostId}`;
            }
        },
        ...
        nextPost() {
            if (this.nextPostId) {
                window.location.href = `/user1/board/gongji/${this.nextPostId}`;
            }
        },
         ...
         */
        backToList() {
            window.location.href = '/user1/board/gongji';
        }
    };
}
